...
class NodeNetwork(nn.Module):
    ...
    """
    def __init__(self, input_dim, output_dim, hidden_activation='Tanh', output_activation='Tanh',
                 layer_norm=True):
        super(NodeNetwork, self).__init__()
        self.network = make_mlp(input_dim*3, [input_dim]*3 + [output_dim],
                                hidden_activation=hidden_activation,
...
class GNN(nn.Module):
    ...
    def __init__(self, input_dim=3, hidden_dim=8, n_graph_iters=3,
        ...
        self.node_network = NodeNetwork(hidden_dim, hidden_dim,
                                        hidden_activation, hidden_activation, layer_norm=layer_norm)
        
        # Node Attention
        self.node_attention = NodeNetwork(hidden_dim, 1,
                                        hidden_activation, None, layer_norm=layer_norm)
    ...
    def forward(self, x, edge_index, batch, batch_size, e=None, is_e=False):
        ...
        node_attention = torch.exp(self.node_attention(x, e, edge_index).squeeze(-1))
        attention_denominator = scatter_add(node_attention[:, None], batch, dim=0, dim_size=batch_size)
        node_attention = node_attention / (attention_denominator[batch].squeeze(-1))
        summary = scatter_add(node_attention[:, None]*node_output, batch, dim=0, dim_size=batch_size)
