...
from models.GNN import GNN_e
from models.encoders import SoftPoolingGcnEncoder

class GraphPooling(nn.Module):
    def __init__(self, GNN_config={}, learning_rate=0.001, lr_scheduler_decrease_rate=0.95, diff_pool_config={}, mid_loss=False, mid_loss_weight=None):
        """
        SetToGraph model.
        ...
        # GNN model
        self.gnn = GNN_e(**GNN_config)
        self.assign_gnn = GNN_e(**GNN_config)
    ...
    def forward(self, x, edge_index, batch, batch_size, e, is_e):
        edge_output, node_output = self.gnn(x, edge_index, e)
        _, node_assign = self.assign_gnn(x, edge_index, e)
        n_hits = x.shape[0] // batch_size
        nodes = node_output.view(batch_size.item(), n_hits, node_output.shape[1]) # BxNxC
        node_assign = torch.softmax(node_assign, dim=-1).view(batch_size.item(), n_hits, node_output.shape[1]) # B*N*C
        A = torch.cuda.FloatTensor(batch_size, n_hits, n_hits).fill_(0)
        start, end = edge_index
        A[batch[start], (start-n_hits*batch[start]), (end-n_hits*batch[start])] = e
        A = 0.5 * A + 0.5 * node_assign @ torch.transpose(node_assign, 1, 2)
