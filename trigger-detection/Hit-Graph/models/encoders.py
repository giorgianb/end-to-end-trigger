...
class GcnEncoderGraph(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            pred_hidden_dims=[], concat=True, bn=False, dropout=0.0, args=None):
        ...
        self.conv_first, self.conv_block, self.conv_last = self.build_conv_layers(
                ...
                add_self, normalize=self.bn, dropout=dropout)
        self.act = nn.Tanh()
    ...
    def build_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers, add_self,
            ...
            # conv_first is a GraphConv (input_dim -> hidden_dim), 
            # conv_block are num_layers-2 GraphConvs (always hidden_dim), 
            # conv_last is a GraphConv (hidden_dim -> embedding_dim)
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            ...
            pred_hidden_dims=[20], concat=True, bn=False, dropout=0.0, linkpred=True,
            assign_input_dim=-1, args=None, assign_dims_list=None, use_assign_dims=False):
        '''
        Args:
        ...
        for i in range(num_pooling):
            ...
            # conv_first2 is a GraphConv (pred_input_dim -> hidden_dim), 
            # conv_block2 are num_layers-2 GraphConvs (always hidden_dim), 
            # conv_last2 is a GraphConv (hidden_dim -> embedding_dim)
        ...
        for i in range(num_pooling):
            # assign_conv_first is a GraphConv (assign_input_dim -> assign_hidden_dim), 
            # assign_conv_block are assign_num_layers-2 GraphConvs (always assign_hidden_dim), 
            # assign_conv_last is a GraphConv (assign_hidden_dim -> assign_dim)
            if use_assign_dims:
                assign_dim = assign_dims_list[i]
            ...
            # assign_pred has 1 layer: assign_pred_input_dim -> assign_dim
        ...
        # pred_model is a 2-layer: pred_input_dim * (num_pooling+1) -> pred_hidden_dims -> label_dim
        self.pred_model = self.build_pred_layers(self.pred_input_dim * (num_pooling+1)+1, pred_hidden_dims, 
                label_dim, num_aggs=self.num_aggs)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        # print('embedding_tensor: ', embedding_tensor.shape, embedding_tensor[0, :5, :])
        ...
        for i in range(self.num_pooling):
            ...
            self.intermediate_result.append(self.assign_tensor)
        ...
        n_hits = torch.cuda.FloatTensor(out.shape[0], 1).fill_(adj.shape[1])
        out_all.append(n_hits)
        if self.concat:
            output = torch.cat(out_all, dim=1)
        else:
            output = out
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1, type='softmax'):
        ''' 
        Args:
            batch_num_nodes: numpy array of number of nodes in each graph in the minibatch.
        '''
        eps = 1e-7
        loss = super(SoftPoolingGcnEncoder, self).loss(pred, label, type=type)
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            ...
            print(adj.shape, pred_adj.shape)
...
class SmallerSoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1,
            pred_hidden_dims=[20], concat=True, bn=False, dropout=0.0, linkpred=True,
