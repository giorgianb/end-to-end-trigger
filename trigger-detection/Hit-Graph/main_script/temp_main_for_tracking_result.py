...
from dataloaders.temp_dataloader_for_tracking_result import get_data_loaders
...
def parse_args():
    ...
    argparser.add_argument('--wandb', type=str, default='trigger-7layer', 
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None, config_file=None):
    ...
    if config['model_name'] == 'Diffpool' or config['model_name'] == 'Diffpool_linkloss':
        ...
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-hd{config['model']['hidden_dim']}-gi{config['model']['n_graph_iters']}-n{config['data']['n_train']*2}"
