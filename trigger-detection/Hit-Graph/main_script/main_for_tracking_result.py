...
from dataloaders.dataloader_for_tracking_result import get_data_loaders
...
def parse_args():
    ...
    argparser.add_argument('config', nargs='?', default='configs/Diffpool.yaml')
    ...
    argparser.add_argument('--wandb', type=str, default='thesis-hitgraph', 
...
def do_epoch(phase, data, model, epoch, output_dir=None, use_wandb=False):
    if phase == 'train':
        model.train()
        if epoch > 1:
    ...
    accum_info = {k: 0.0 for k in ['ri', 'loss', 'mincutloss1', 'mincutloss2', 'ortholoss1', 'ortholoss2', 'insts', 'accuracy', 'fscore', 'precision', 'recall', 'label_acc', 'CEloss', 'Glaploss', 'lr']}
    accum_info['lr'] = model.scheduler.get_last_lr()[0]
    preds = []
    predictions = []
    labels = []
    event_labels = []
    ...
    for batch in data:
        ...
        # track_cumsum_temp = np.cumsum(n_tracks)
        # track_cumsum = track_cumsum_temp.clone()
        # track_cumsum[1:] = track_cumsum_temp[:-1]
        # track_cumsum[0] = 0
        # ip = batch.ip
        # if model.name == 'GNNDiffpool':
        #     hits = torch.cat((hits, n_hits[batch.batch].view(-1, 1)), dim=1)
        
        # if model.name == 'GNN_Diffpool_trackinfo':
        #     hits = torch.cat((hits, n_hits[batch.batch].view(-1, 1), batch.tracks_info[track_cumsum[batch.batch]+hits_to_track,9:]), dim=1)
        ...
        # track_cumsum = track_cumsum.to(DEVICE, torch.long)
        ...
        # ip = ip.view(batch_size, 3).to(DEVICE, torch.float)
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            elif model.name == 'PVCNN':
                n_hits = hits.shape[0] // batch_size
                hits = hits.view(batch_size.item(), n_hits, hits.shape[1])
                ip_pred, _ = model(hits)
            elif model.name == 'MinCutPool':
                ip_pred, mincut_loss1, ortho_loss1, mincut_loss2, ortho_loss2 = model(hits, edge_index, batch.batch, batch_size, e, is_e=True)
            elif model.name == 'Diffpool_linkloss':
                ip_pred, A = model(hits, edge_index, batch.batch, batch_size, e, is_e=True)
            else:
                ip_pred = model(hits, edge_index, batch.batch, batch_size, e, is_e=True)
            if model.name == 'SAGPool':
                pred = ip_pred.max(dim=1)[1]
                preds.extend(pred.cpu().data.numpy())
                predictions.extend(nn.Softmax(dim=1)(ip_pred)[:, 1].detach().cpu().numpy().flatten())
            else:
                ip_pred = ip_pred.squeeze(1)
                preds.append((ip_pred>0).cpu().data.numpy())
                predictions.append(ip_pred.cpu().data.numpy())
            ...
            if phase == 'train':
                if model.name == 'GNNDiffpool' and model.mid_loss:
                    ...
                    loss = model.train_model(ip_pred, trig, mincut_loss1, ortho_loss1, mincut_loss2, ortho_loss2)
                elif model.name == 'Diffpool_linkloss':
                    loss = model.train_model(ip_pred, trig, A)
                else:
                ...
                if model.name == 'GNNDiffpool' and model.mid_loss:
                    ...
                    loss = model.get_loss(ip_pred, trig, mincut_loss1, ortho_loss1, mincut_loss2, ortho_loss2)
                elif model.name == 'Diffpool_linkloss':
                    loss = model.get_loss(ip_pred, trig, A)
                else:
        ...
        if model.name == 'MinCutPool':
            accum_info['mincutloss1'] += mincut_loss1
            accum_info['mincutloss2'] += mincut_loss2
            accum_info['ortholoss1'] += ortho_loss1
            accum_info['ortholoss2'] += ortho_loss2
    ...
    accum_info['auroc'] = metrics.roc_auc_score(labels, predictions)
    ...
    if model.name == 'MinCutPool':
        logging.info(f"total loss: {loss} mincut loss1: {accum_info['mincutloss1']} ortho loss1: {accum_info['ortholoss1']}\n mincut loss2: {accum_info['mincutloss2']} ortho loss2: {accum_info['ortholoss2']}")
    ...
    if use_wandb:
        model_name = 'Diffpool'
        wandb.log({phase.capitalize() + " Learning Rate for " + model_name.capitalize(): accum_info['lr']})
        wandb.log({phase.capitalize() + " Loss for " + model_name.capitalize(): accum_info['loss']})
        wandb.log({phase.capitalize() + " Accuracy for " + model_name.capitalize(): accum_info['label_acc']})
        wandb.log({phase.capitalize() + " Precion for " + model_name.capitalize(): result['prec']})
        wandb.log({phase.capitalize() + " Recall for " + model_name.capitalize(): result['recall']})
        wandb.log({phase.capitalize() + " F Score for " + model_name.capitalize(): result['F1']})
        wandb.log({phase.capitalize() + " Roc_auc for " + model_name.capitalize(): accum_info['auroc']})
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None, config_file=None):
    ...
    # Connect to wandb
    if config['model_name'] == 'Diffpool' or config['model_name'] == 'Diffpool_linkloss':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-b{config['data']['batch_size']}-id{config['model']['hidden_dim']}-d{config['model']['diff_pool_config']['hidden_dim']}-ed{config['model']['diff_pool_config']['embedding_dim']}-ahd{config['model']['diff_pool_config']['assign_hidden_dim']}-adl{config['model']['diff_pool_config']['assign_dims_list']}-n_pool{config['model']['diff_pool_config']['num_pooling']}-n{config['data']['n_train']*2}"
    elif config['model_name'] == 'GCNPooling':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-b{config['data']['batch_size']}-id{config['model']['input_dim']}-od{config['model']['output_dim']}-add_self{config['model']['add_self']}-agg{config['model']['agg']}"
    elif config['model_name'] == 'GNNPooling_Nodeonly' or config['model_name'] == 'GNNPooling' or config['model_name'] == 'GNNPooling_Att':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-hd{config['model']['hidden_dim']}-gi{config['model']['n_graph_iters']}"
    elif config['model_name'] == 'GNN_Diffpool':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-hd{config['model']['GNN_config']['hidden_dim']}-gi{config['model']['GNN_config']['n_graph_iters']}-dd{config['model']['diff_pool_config']['hidden_dim']}-ed{config['model']['diff_pool_config']['embedding_dim']}-ahd{config['model']['diff_pool_config']['assign_hidden_dim']}-adl{config['model']['diff_pool_config']['assign_dims_list']}-n_pool{config['model']['diff_pool_config']['num_pooling']}-n{config['data']['n_train']*2}"
    elif config['model_name'] == 'SAGPool':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-b{config['data']['batch_size']}-H_{config['model']['is_hierarchical']}-hd{config['model']['nhid']}-pratio{config['model']['pooling_ratio']}-dropout{config['model']['dropout_ratio']}"
    elif config['model_name'] == 'GraphPooling':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-hd{config['model']['GNN_config']['hidden_dim']}-gi{config['model']['GNN_config']['n_graph_iters']}-dd{config['model']['diff_pool_config']['hidden_dim']}-ed{config['model']['diff_pool_config']['embedding_dim']}-ahd{config['model']['diff_pool_config']['assign_hidden_dim']}-adl{config['model']['diff_pool_config']['assign_dims_list']}-n_pool{config['model']['diff_pool_config']['num_pooling']}-n{config['data']['n_train']*2}"
    elif config['model_name'] == 'PVCNN':
        name = f"{config['model_name_on_wandb']}-lr{config['model']['learning_rate']}-h{config['model']['out_channels']}"
    elif config['model_name'] == 'MinCutDiffpool':
        name = f"{config['model_name_on_wandb']}-mincut_dim{config['model']['mincut_dim']}-lr{config['model']['learning_rate']}-b{config['data']['batch_size']}-id{config['model']['hidden_dim']}-n{config['data']['n_train']*2}"
    ...
    if config['model_name'] == 'Diffpool_linkloss':
        from models.Diffpool_linkloss import Diffpool_linkloss
        model = Diffpool_linkloss(**config['model'])
    ...
    if config['model_name'] == 'GCNPooling':
        from models.GCN import GraphConvPooling
        model = GraphConvPooling(**config['model'])
    if config['model_name'] == 'GNNPooling_Nodeonly':
        from models.GNNPooling_Nodeonly import GNN
        model = GNN(**config['model'])
    if config['model_name'] == 'GNNPooling':
        from models.GNNPooling import GNN
        model = GNN(**config['model'])
    if config['model_name'] == 'GNNPooling_Att':
        from models.GNNPooling_Att import GNN
        model = GNN(**config['model'])
    if config['model_name'] == 'SAGPool':
        from models.SAGPool import SAGPoolNet
        model = SAGPoolNet(**config['model'])
    if config['model_name'] == 'GraphPooling':
        from models.GraphPooling import GraphPooling
        model = GraphPooling(**config['model'])
    if config['model_name'] == 'PVCNN':
        from models.PVCNN import PVConv
        model = PVConv(**config['model'])
    if config['model_name'] == 'MinCutDiffpool':
        from models.MinCutPool import MinCutDiffpool
        model = MinCutDiffpool(**config['model'])
    ...
    if auto:
        ...
        train_data, val_data, test_data = get_data_loaders(**config['data'], model_name=model.name)
...
#          print(itm.shape)
    ...
    for epoch in range(1, config['epochs'] + 1):
        train_info = do_epoch('train', train_data, model, epoch, config['output_dir'], use_wandb=args.use_wandb)
        ...
        val_info = do_epoch('valid', val_data, model, epoch, use_wandb=args.use_wandb)
 ...
 #######################################################################################################")
        ...
        if val_info['label_acc'] > best_val_acc:
            best_val_acc = val_info['label_acc']
            best_epoch = epoch
            best_model = copy.deepcopy(model)

    logging.info(f'Best validation acc: {best_val_acc:.4f}, best epoch: {best_epoch}.')

    logging.info(f'Training runtime: {str(datetime.now() - start_time).split(".")[0]}')

    test_info = do_epoch('valid', test_data, best_model, 0, use_wandb=False)
    logging.info(f"test total loss:{test_info['loss']:.6f}  --acc: {test_info['label_acc']:.4f} -- runtime:{test_info['run_time']}\n \
 #######################################################################################################")
