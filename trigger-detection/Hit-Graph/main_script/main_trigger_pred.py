...
def do_epoch(phase, data, model, epoch, output_dir=None):
    if phase == 'train':
        model.train()
        if epoch > 1:
...
#          print(itm.shape)
    ...
    for epoch in range(1, config['epochs'] + 1):
        train_info = do_epoch('train', train_data, model, epoch, config['output_dir'])
        ...
        val_info = do_epoch('valid', val_data, model, epoch)
