...
class HitGraphDataset(Dataset):
    ...
    def __init__(self, filenames, real_weight=1.0, false_weight=1.0, random_permutation=True):
...
def get_datasets(n_train, n_valid, n_test, input_dir1, input_dir2, 
                real_weight=1.0, false_weight=1.0, random_permutation=True):
    ...
    filenames2 = sorted([os.path.join(input_dir2, f) for f in os.listdir(input_dir1)
                                if f.startswith('event') and not f.endswith('_ID.npz')])[:n_train+n_valid+n_test]
    ...
    train_data = HitGraphDataset(filenames = train_filenames,
                           ...
                           false_weight=false_weight, random_permutation=random_permutation)
