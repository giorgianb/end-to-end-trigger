...
def load_graph(filename):
    ...
    with np.load(filename) as f:
        ...
        e = f['e']
        # print(edge_index.shape, e.shape)
        # 408 for 800k
        hits_fix = np.zeros([726, 4])
        n_hit = hits.shape[0]
        hits_fix[:n_hit, :] = hits
        # 1388 for 800k
        edge_index_fix = np.zeros([2, 4106])
        n_edge = edge_index.shape[1]
        edge_index_fix[:, :n_edge] = edge_index

        e_fix = np.zeros([4106, 1])
        e_fix[:n_edge, :] = e
    return hits_fix, edge_index_fix, trigger_flag, e_fix, n_edge, n_hit
...
for file_index in range(len(filenames)):
    hits, edge_index, trigger_flag, e, n_edge, n_hit = load_graph(filenames[file_index])
...
data_to_save = {
    'hits_info_15': hits_info_15,
    'edge_info_15': edge_info_15
}
