...
# local
from .utils_settrans import Masked_SAB_PT
...
class Particle_Transformer(nn.Module):
    ...
    def generate_embedding(self, X, U, mask):
        if self.recalculate_hits_mean:
            X = self._recalculate_hits_mean(X, mask)
        H = self._generate_track_embeddings(X, U, mask)
        if self.final_pooling:
    ...
    def _generate_track_embeddings(self, X, U, mask):
        for layer in self._layers:
            pred_x, agg = layer(X, U, mask)
            X = pred_x
...
class Bipartite_Layers(nn.Module):
    def __init__(self,
        ...
        self.enc = nn.ModuleList([
            Masked_SAB_PT(input_dim, feature_dim, ln=ln),
            Masked_SAB_PT(feature_dim, feature_dim, ln=ln),
        ])
    ...
    def forward(self, X, U, mask):
        ...
        """
        Xp = self.enc[0](X, U, mask) * mask.unsqueeze(-1)
        Xp = self.enc[1](Xp, U, mask) * mask.unsqueeze(-1)
