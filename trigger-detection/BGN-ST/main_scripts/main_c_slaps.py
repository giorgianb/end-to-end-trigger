...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/c_slaps.yaml')
...
def train(data, model, dae_weight, optimizer, epoch, output_dir, trigger_data, nontrigger_data, contrast_loss_fn, n_positive, n_negative, drop_probability):
    train_info = do_epoch(data, model,  dae_weight, trigger_data, nontrigger_data, contrast_loss_fn,  n_positive, n_negative, drop_probability, optimizer)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info


def evaluate(data, model, dae_weight, trigger_data, nontrigger_data, contrast_loss_fn, n_positive, n_negative, drop_probability):
    with torch.no_grad():
        val_info = do_epoch(data, model, dae_weight, trigger_data, nontrigger_data, contrast_loss_fn, n_positive, n_negative, drop_probability, optimizer=None)
    return val_info

def do_epoch(data, model, dae_weight, trigger_data, nontrigger_data, contrast_loss_fn, n_positive, n_negative, drop_probability, optimizer=None):
    ...
    mse_loss = nn.MSELoss()
    for batch, trigger_batch, nontrigger_batch in tqdm.tqdm(zip(data, trigger_data, nontrigger_data), total=len(trigger_data)):
...
def main():
    ...
    for epoch in range(1, config['epochs'] + 1):
        train_info = train(train_data, model, config['slaps']['dae_weight'], optimizer, epoch, config['output_dir'], trigger_train_data, nontrigger_train_data, contrast_loss_fn,  n_negative=n_negative, n_positive=n_positive, drop_probability=drop_probability)
        table = make_table(
        ...
        val_info = evaluate(
                data=val_data, 
                model=model, 
                dae_weight=config['slaps']['dae_weight'], 
                trigger_data=trigger_val_data, 
                nontrigger_data=nontrigger_val_data, 
                contrast_loss_fn=contrast_loss_fn, 
    ...
    test_info = evaluate(
        ...
        model=model, 
        dae_weight=config['slaps']['dae_weight'], 
        trigger_data=trigger_test_data, 
        nontrigger_data=nontrigger_test_data, 
        contrast_loss_fn=contrast_loss_fn, 
        n_negative=n_negative, 
        n_positive=n_positive, 
        drop_probability=drop_probability
    ...
    if args.use_wandb:
        ...
        wandb.log({"Contrastive Test Loss" : test_info['loss_contrast']})
