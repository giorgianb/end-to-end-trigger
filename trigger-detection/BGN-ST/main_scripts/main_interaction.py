...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/interaction.yaml')
...
def do_epoch(data, model, loss_params, epoch, optimizer=None):
    ...
    for batch, mask in tqdm.tqdm(data, smoothing=0.0):
        ...
        loss = 0
        mse_loss_physics = F.mse_loss(pred_perturbed, physics)
        loss += mse_loss_physics
        accum_info['loss_mse_physics'] += mse_loss_physics.item() * batch_size
        tracks = batch.track_vector.to(DEVICE)
        embeddings_unperturbed, pred_unperturbed = model.generate_track_embeddings(tracks, mask)
        loss_mse_physics = F.mse_loss(pred_unperturbed, physics)
        loss += loss_mse_physics
        accum_info['loss_mse_physics'] += loss_mse_physics.item() * batch_size
    ...
    if num_insts > 0:
        ...
        accum_info['loss_mse_physics'] /= num_insts
...
def execute_training(args, config):
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        table = make_table(
            ...
            ('Physics loss', f"{train_info['loss_mse_physics']:.6f}"),
        ...
        train_loss[epoch-1], train_ri[epoch-1] = train_info['loss'], train_info['loss_mse_physics']
        if args.use_wandb:
            wandb.log({"Train Loss" : train_info['loss']}, step=epoch)
            wandb.log({"Train Adj Loss" : train_info['loss_adj']}, step=epoch)
            wandb.log({"Train Physics Loss" : train_info['loss_mse_physics']}, step=epoch)
        ...
        table = make_table(
            ...
            ('Physics loss', f"{val_info['loss_mse_physics']:.6f}"),
        ...
        if val_info['loss_mse_physics'] > best_classifier_val_ri:
            best_classifier_val_ri = val_info['loss_mse_physics']
            best_model = copy.deepcopy(model)

        if val_info['loss_mse_physics'] > best_val_ri:
            best_val_ri = val_info['loss_mse_physics']
            best_epoch = epoch
            best_model = copy.deepcopy(model)

        val_loss[epoch-1], val_ri[epoch-1] = val_info['loss'], val_info['loss_mse_physics']
        if args.use_wandb:
            wandb.log({"Validation Loss" : val_info['loss']}, step=epoch)
            wandb.log({"Validation Adj Loss" : val_info['loss_adj']}, step=epoch)
            wandb.log({"Validation Physics Loss" : val_info['loss_mse_physics']}, step=epoch)
            wandb.log({"Best Classifier Physics Loss": best_classifier_val_ri}, step=epoch)
    ...
    table = make_table(
        ...
        ('Physics loss', f"{test_info['loss_mse_physics']:.6f}"),
    ...
    if args.use_wandb:
        ...
        wandb.log({"Test Physics Loss" : test_info['loss_mse_physics']})
