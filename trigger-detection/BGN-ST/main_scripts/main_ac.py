...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/ac.yaml')
...
def do_contrast_epoch(trigger_data, nontrigger_data, model, contrast_loss_fn, epoch, n_positive, n_negative, drop_probability, optimizer=None):
    ...
    for (trigger_batch, trigger_mask), (nontrigger_batch, nontrigger_mask) in tqdm.tqdm(zip(trigger_data, nontrigger_data), total=len(trigger_data)):
        loss = 0
        ...
        trigger_node = trigger_batch.trigger_node.to(DEVICE).squeeze(-1).to(torch.bool)

        trigger_mask = trigger_mask.to(DEVICE).to(torch.bool)
        nontrigger_mask = nontrigger_mask.to(DEVICE).to(torch.bool)
        ...
        trigger_pos_mask = torch.cat([trigger_pos_mask, trigger_node.unsqueeze(1)], dim=1)


        # Ensure all trigger tracks are included
        trigger_pos_mask = trigger_pos_mask | trigger_node.unsqueeze(1)
        ...
        trigger_neg_mask = torch.cat([trigger_neg_mask, ~trigger_node.unsqueeze(1)], dim=1)

        # Ensure that no trigger tracks are included
        trigger_neg_mask = trigger_neg_mask & ~trigger_node.unsqueeze(1)
...
def execute_training(args, config):
    ...
    dconfig = copy.copy(config['data'])
    dconfig['trigger_input_dir'] = None
