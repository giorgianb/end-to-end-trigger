...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/gttrk_masked.yaml')
...
def execute_training(args, config):
    ...
    if config['optimizer']['type'] == 'Adam':
        ...
        optimizer = torch.optim.SGD(params=model.parameters(), lr=oconfig['learning_rate'], momentum=oconfig['momentum'], weight_decay=oconfig['weight_decay'])
    else:
        raise NotImplementedError(f'Optimizer {config["optimizer"]["type"]} not implemented.')


    decay_rate = config["optimizer"]["learning_rate_decay_rate"]
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        if args.early_stopping and epoch >= args.early_stopping_epoch and best_val_ri < args.early_stopping_accuracy:
            break
    ...
    if args.use_wandb:
        ...
        wandb.log({"Test Loss" : test_info['loss_ce']})
