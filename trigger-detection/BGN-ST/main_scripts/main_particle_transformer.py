...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/particle_transformer.yaml')
...
def train(data, model, physics_model, loss_params, optimizer, epoch, output_dir):
    train_info = do_epoch(data, model, physics_model, loss_params, epoch, optimizer=optimizer)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model, physics_model, loss_params, epoch):
    with torch.no_grad():
        val_info = do_epoch(data, model, physics_model, loss_params, epoch, optimizer=None)
    return val_info

def do_epoch(data, model, physics_model, loss_params, epoch, optimizer=None):
    ...
    for batch, mask in tqdm.tqdm(data, smoothing=0.0):
        ...
        physics = physics_model(tracks, mask)
        momentum = physics[..., :3]
        energy = physics[..., 3:4]
        ...
        U = torch.stack([delta, k_T, z, m], dim=-1)
        U[torch.any(torch.isnan(U))] = 0
        U = U.detach()
        ...
        embeddings_perturbed, pred_perturbed = model.generate_track_embeddings(tracks, U, mask)
        loss = 0
        ...
        embeddings_unperturbed, pred_unperturbed = model.generate_track_embeddings(tracks, U, mask)
...
def execute_training(args, config):
    ...
    with open('train_results/biatt-particle-transformer/experiment_2024-02-09_22:33:31/config.pkl', 'rb') as f:
    ...
    pconfig_path = config['physics_model_config_path']
    with open(pconfig_path, 'rb') as f:
        physics_config = pickle.load(f)
        pmconfig = physics_config['model']

    from models.Bipartite_Attention_Masked import Bipartite_Attention as Physics_Model
    from models.Particle_Transformer import Particle_Transformer as Model

    checkpoint_file = config['physics_model_checkpoint_path']
    physics_model = Physics_Model(**pmconfig)
    physics_model = load_checkpoint(checkpoint_file, physics_model)
    ...
    # TODO: remove, just for the purpose of further optimizing
    checkpoint_file = '/home1/giorgian/projects/trigger-detection-pipeline/sPHENIX/trigger-detection/BGN-ST/train_results/biatt-particle-transformer/experiment_2024-02-09_22:33:31/checkpoints/model_checkpoint_016.pth.tar'
    model, optimizer = load_checkpoint(checkpoint_file, model, optimizer)
    ...
    for epoch in range(1, config['epochs'] + 1):
        train_info = train(train_data, model, physics_model, config['loss'], optimizer, epoch, config['output_dir'])
        table = make_table(
        ...
        val_info = evaluate(val_data, model, physics_model, config['loss'], epoch)
        table = make_table(
    ...
    test_info = evaluate(test_data, best_model, physics_model, config['loss'], config['epochs'] + 1)
    del tmp_dir
    table = make_table(
