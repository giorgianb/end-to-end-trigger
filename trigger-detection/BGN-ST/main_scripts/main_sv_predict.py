...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/sv_predict.yaml')
...
def do_epoch(data, model, loss_params, epoch, optimizer=None):
    ...
    for batch, mask in tqdm.tqdm(data):
        ...
        loss = 0
        _, t_preds, ovs = model.generate_track_embeddings(tracks, mask)
        ce_loss = F.cross_entropy(t_preds, trig)
        loss += ce_loss
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        loss_mse = F.mse_loss(ovs, origin_vertices)
        loss += mse_weight*loss_mse
        accum_info['loss_mse'] += loss_mse.item() * batch_size

        if optimizer is not None:
            pred = t_preds.max(dim=1)[1]
            preds.extend(pred.cpu().data.numpy())
            preds_prob.extend(nn.Softmax(dim=1)(t_preds)[:, 1].detach().cpu().numpy().flatten())
        ...
        if optimizer is not None:
            accum_info = calc_metrics(trig, t_preds, accum_info)
        accum_info = calc_metrics(trig, t_preds, accum_info)
...
def execute_training(args, config):
    ...
    from models.Bipartite_Attention_SV import Bipartite_Attention as Model
