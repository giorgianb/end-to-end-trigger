...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/glri_2d.yaml')
...
def train(data, model, lri_params, optimizer, epoch, output_dir, finetune_epoch):
    train_info = do_epoch(data, model, lri_params, epoch, finetune_epoch=finetune_epoch, optimizer=optimizer)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model, lri_params, epoch, finetune_epoch):
    with torch.no_grad():
        val_info = do_epoch(data, model, lri_params, epoch, optimizer=None, finetune_epoch=finetune_epoch)
    return val_info

def do_epoch(data, model, lri_params, epoch, finetune_epoch=False, optimizer=None):
    ...
    total_size = 0
    total_selected = 0
    sigma, beta = lri_params['sigma'], lri_params['beta']
    for batch in tqdm.tqdm(data):
        ...
        ov = batch.origin_vertices.to(DEVICE)
        ...
        pred_labels, pred_sigma, good_hits = model(tracks, mask, ov, finetune_epoch=finetune_epoch)
        loss = 0
        ce_loss = F.cross_entropy(pred_labels, trig)
        loss += ce_loss
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        if finetune_epoch:
            true_sigma = sigma * torch.eye(pred_sigma.shape[-1], device=DEVICE).reshape(1, 1, 1, 2, 2).repeat(
...
def execute_training(args, config):
    ...
    from models.Bipartite_Attention_gLRI2d_single_hit import Bipartite_Attention as Model
    ...
    params = [
            {'params': chain(model.interpreter.parameters(), model.noiser.parameters()), 'lr': oconfig['learning_rate']*1e-4},
    ...
    for epoch in range(1, config['epochs'] + config['finetune_epochs'] + 1):
        ...
        train_info = train(train_data, model, config['lri'], optimizer, epoch, config['output_dir'], finetune_epoch=finetune_epoch)
        table = make_table(
        ...
        val_info = evaluate(val_data, model, config['lri'], epoch, finetune_epoch=finetune_epoch)
        table = make_table(
    ...
    test_info = evaluate(test_data, best_model, config['lri'], config['epochs'] + 1, finetune_epoch=True)
    table = make_table(
