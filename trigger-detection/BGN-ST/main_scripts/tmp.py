...
def do_epoch(data, model, loss_params, epoch, finetune_epoch=False, optimizer=None):
    ...
    for batch in tqdm.tqdm(data):
        ...
        pred_perturbed, embeddings_perturbed = model.get_track_embeddings(tracks, mask)
        loss = 0
        ce_loss = F.cross_entropy(pred_perturbed, trig)
        loss += ce_loss
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        tracks = batch.track_vector.to(DEVICE)
        pred_unperturbed, embeddings_unperturbed = model.get_track_embeddings(tracks, mask)
