...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/augment_contrast.yaml')
...
def do_epoch(data, model, loss_params, epoch, optimizer=None):
    ...
    total_size = 0
    total_selected = 0
    mse_weight, adj_weight, con_weight, pos_margin, neg_margin = loss_params['mse_weight'], loss_params['adj_weight'], loss_params['con_weight'], loss_params['pos_margin'], loss_params['neg_margin']
    def augment_adj(tracks, trigger, track_origin, mask, update_stats=False, use_cross_entropy=True):
        batch_size = tracks.shape[0]
        orig_tracks = torch.clone(tracks)
        ...
        if optimizer is not None and update_stats:
        ...
        event_embedding = model.get_event_embedding(embeddings_unperturbed, mask)

        return pred_unperturbed, event_embedding, loss




    contrastive_loss = losses.ContrastiveLoss(pos_margin=pos_margin, neg_margin=neg_margin)
    for batch, gt_mask, pred_mask in tqdm.tqdm(data, smoothing=0.0):
        gt_tracks = batch.gt_track_vector.to(DEVICE, torch.float)
        gt_trigger = (batch.gt_trigger.to(DEVICE) == 1).long()
        gt_track_origin = batch.gt_track_origin.to(DEVICE, torch.float)
        gt_mask = gt_mask.to(DEVICE, torch.float)

        pred_tracks = batch.pred_track_vector.to(DEVICE, torch.float)
        pred_trigger = (batch.pred_trigger.to(DEVICE) == 1).long()
        pred_track_origin = batch.pred_track_origin.to(DEVICE, torch.float)
        pred_mask = pred_mask.to(DEVICE, torch.float)


        gt_pred_unperturbed, gt_event_embeddings, gt_loss = augment_adj(gt_tracks, gt_trigger, gt_track_origin, gt_mask, update_stats=False, use_cross_entropy=False)
        pred_pred_unperturbed, pred_event_embeddings, pred_loss = augment_adj(pred_tracks, pred_trigger, pred_track_origin, pred_mask, update_stats=True, use_cross_entropy=True)
        

        labels = torch.arange(gt_tracks.shape[0]).to(DEVICE)
        labels = torch.cat([labels, labels])
        embeddings = torch.cat([gt_event_embeddings, pred_event_embeddings])
        con_loss = contrastive_loss(embeddings, labels)
        accum_info['loss_con'] += con_loss.item() * gt_tracks.shape[0]

        loss = gt_loss + pred_loss + con_weight*con_loss
        if optimizer is not None:
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

        accum_info = calc_metrics(pred_trigger, pred_pred_unperturbed, accum_info)
        accum_info['loss'] += loss.item()
        num_insts += gt_tracks.shape[0]
...
def execute_training(args, config):
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        table = make_table(
            ...
            ('Con loss', f"{train_info['loss_con']:.6f}"),
        ...
        if args.use_wandb:
            ...
            wandb.log({"Train Con Loss" : train_info['loss_con']}, step=epoch)
        ...
        table = make_table(
            ...
            ('Con loss', f"{val_info['loss_con']:.6f}"),
        ...
        if args.use_wandb:
            ...
            wandb.log({"Validation Con Loss" : val_info['loss_con']}, step=epoch)
    ...
    table = make_table(
        ...
        ('Con loss', f"{test_info['loss_con']:.6f}"),
    ...
    if args.use_wandb:
        ...
        wandb.log({"Test Con Loss" : test_info['loss_con']})
