...
def do_epoch(data, model, loss_params, epoch, optimizer=None):
    ...
    total_size = 0
    total_selected = 0
    mse_weight, adj_weight, tn_weight = loss_params['mse_weight'], loss_params['adj_weight'], loss_params['tn_weight']
    for batch, mask in tqdm.tqdm(data):
        tracks = batch.track_vector.to(DEVICE, torch.float)
        trig = (batch.trigger.to(DEVICE) == 1).long()
        trigger_node = batch.trigger_node.to(DEVICE, torch.bool)
        origin_vertices = batch.track_origin.to(DEVICE, torch.float)
        ...
        embeddings_perturbed, pred_perturbed, tn_perturbed = model.generate_track_embeddings(tracks, mask)
        loss = 0
        ce_loss = F.cross_entropy(pred_perturbed, trig)
        loss += ce_loss
        tn_loss = F.cross_entropy(tn_perturbed[mask.to(torch.bool)], trigger_node[mask.to(torch.bool)].to(torch.long).squeeze(-1))
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        loss += tn_loss*tn_weight
        tracks = batch.track_vector.to(DEVICE)
        embeddings_unperturbed, pred_unperturbed, tn_unperturbed = model.generate_track_embeddings(tracks, mask)
        ce_loss = F.cross_entropy(pred_unperturbed, trig)
        loss += ce_loss
        accum_info['loss_ce'] += ce_loss.item() * batch_size
        tn_loss = F.cross_entropy(tn_unperturbed[mask.to(torch.bool)], trigger_node[mask.to(torch.bool)].to(torch.long).squeeze(-1))
        loss += tn_loss*tn_weight
        ...
        if adj_weight != 0:
            # origin_vertices: (batch, track, 3)
            c1 = origin_vertices.unsqueeze(-2) # (batch, track, 1, 3)
            c2 = origin_vertices.unsqueeze(-3) # (batch, 1, track, 3)
        ...
        t_only_mask = mask * trigger_node.squeeze(-1)
        _, pred_t_only, tn_t_only = model.generate_track_embeddings(tracks, t_only_mask)
        tn_loss_t_only = F.cross_entropy(tn_t_only[t_only_mask.to(torch.bool)], trigger_node[t_only_mask.to(torch.bool)].to(torch.long).squeeze(-1))
        loss += tn_loss_t_only*tn_weight
        ce_loss_t_only = F.cross_entropy(pred_t_only, trig)
        loss += ce_loss_t_only
        ...
        if optimizer is not None:
            accum_info = calc_metrics(trig, pred_perturbed, accum_info)
        accum_info = calc_metrics(trig, pred_unperturbed, accum_info)
