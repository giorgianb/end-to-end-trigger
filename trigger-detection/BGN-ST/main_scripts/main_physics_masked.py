...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/ecml_trigger_mlp_masked.yaml')
    ...
    argparser.add_argument('--name', type=str, default=None,
            help="Run name")
    argparser.add_argument('--wandb', type=str, default='trigger-ecml-finalversion', 
...
def train(data, model, model_physics, optimizer, epoch, output_dir, use_radius=False):
    train_info = do_epoch(data, model, model_physics, epoch, optimizer, use_radius=use_radius)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model, model_physics, epoch, use_radius=False):
    with torch.no_grad():
        val_info = do_epoch(data, model, model_physics, epoch, optimizer=None, use_radius=use_radius)
    return val_info


def do_epoch(data, model, model_physics, epoch, optimizer=None, use_radius=False):
    ...
    for batch in data:
        tracks, n_tracks, trig, momentum = batch
        # permutation = np.random.permutation(tracks.shape[1])
        # tracks = tracks[:, permutation]
        tracks = tracks.to(DEVICE, torch.float)
        n_tracks = n_tracks.to(DEVICE)
        trig = (trig.to(DEVICE) == 1).long()
        batch_size = tracks.shape[0]

        pred_momentum = model_physics(tracks)
        tracks = torch.concat([tracks[:, :, :-1], pred_momentum], dim=-1)
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    name = config['name_on_wandb'] + f"-n_hid{config['model']['layers_spec'][0][0]}-n_agg{config['model']['layers_spec'][0][1]}-n_layers{len(config['model']['layers_spec'])}-agg_activation{config['model']['aggregator_activation']}-ln{config['model']['ln']}-use_radius{config['data']['use_radius']}-ntrain{config['data']['n_train1']}-lr{config['optimizer']['learning_rate']}-{config['optimizer']['type']}-b{config['data']['batch_size']}"
    logging.info(name)
    if args.use_wandb:
        wandb.init(
