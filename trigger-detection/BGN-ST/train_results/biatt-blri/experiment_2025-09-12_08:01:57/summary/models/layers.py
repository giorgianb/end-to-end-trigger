...
class Masked_LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps = 1e-5, elementwise_affine = True, device=None, dtype=None):
        ...
        if not isinstance(normalized_shape, int):
            raise NotImplementedError("Masked_LayerNorm does not yet support multiple normalization dimensions")

        self.ln = nn.LayerNorm(normalized_shape, eps=eps, elementwise_affine=elementwise_affine, device=device, dtype=dtype)

    def forward(self, X, mask):
        return self.ln(X)*mask.unsqueeze(-1)
