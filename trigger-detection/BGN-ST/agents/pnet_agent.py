...
import main_scripts.main_pnet as pnet
sweep_id ="1e8aua80"
...
def train():
    print(f'GPU: {GPU}')
    wandb.init(tags=["pnet", "loopy", "layernorm", "batchnorm", "prelu", "4-layer"])
    config = {}
    ...
    k = config['model']['k']
    del config['model']['k']
    config['model']['layers_spec'] = [[64, 3, k], [64, 3, k], [64, 3, k], [64, 3, k]]
    config['wandb'] = {}
    config['wandb']['project_name'] = 'physics-trigger'
    config['wandb']['run_name'] = 'pnet'
    config['wandb']['tags'] = ["pnet", "loopy", "layernorm", "batchnorm", "prelu", "4-layer"]
    ...
    pnet.execute_training(args, config)
