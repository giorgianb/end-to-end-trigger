...
def load_graph(filename, min_edge_probability, intt_required=False, use_true_edges=False):
    ...
    with np.load(filename, allow_pickle=True) as f:
        if use_true_edges:
            ...
            good_edges = particle_id[start] == particle_id[end]
            edge_index = edge_index[:, good_edges]
        else:
...
class TrackDataset(object):
    ...
    def __init__(
        ...
        if trigger_input_dir is not None:
            ...
            print(f'{n_trigger_samples=} {len(filenames)=}')
        ...
        if nontrigger_input_dir is not None:
            ...
            print(f'{n_trigger_samples=} {len(filenames)=} {len(self.filenames)=}')
        ...
        self.intt_required = intt_required
        self.use_true_edges = use_true_edges

    def __getitem__(self, file_index):
        event_info = load_graph(self.filenames[file_index], self.min_edge_probability, self.intt_required, self.use_true_edges)
...
def get_datasets(n_train, n_valid, n_test, 
    ...
    print(f'{len(data)=} {total*(n_train + n_test + n_valid)=}')
