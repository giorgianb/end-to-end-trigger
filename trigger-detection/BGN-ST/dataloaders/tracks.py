...
class EventInfo:
    ...
    track_embeddings: Union[np.ndarray, torch.Tensor]   # NEW
...
def load_graph(filename, noise_level=0):
    ...
    with np.load(filename) as f:
        ...
        # NEW: optional per-track embeddings
        if 'track_embeddings' in f:
            track_embeddings = f['track_embeddings'].astype(np.float32)
        else:
            track_embeddings = np.zeros((track_hits.shape[0], 0), dtype=np.float32)
        ...
        if track_hits.shape[0] == 0:
            ...
            hit_type = np.zeros((0, 5, 3))
            track_embeddings = np.zeros((0, 0), dtype=np.float32)
        ...
        return EventInfo(
                ...
                track_embeddings=track_embeddings,  # NEW
...
class TrackDataset(object):
    ...
    def __init__(
        ...
        self.use_track_hits = use_track_hits

    def __getitem__(self, file_index):
        event_info = load_graph(self.filenames[file_index], noise_level=self.noise_level)

        # Start with empty feature matrix (N x 0) and add pieces conditionally
        n_tracks = event_info.track_hits.shape[0]
        track_vector = np.zeros((n_tracks, 0), dtype=np.float32)
        ...
        # Optionally include raw track hits (scaled)
        if self.use_track_hits:
            track_vector = np.concatenate([track_vector, event_info.track_hits / self.rescale_factor], axis=-1)

        # Optionally include learned embeddings
        if self.use_track_embeddings:
            # event_info.track_embeddings may be (N, 0) if absent in the file, which is safe to concat
            track_vector = np.concatenate([track_vector, event_info.track_embeddings], axis=-1)
        ...
        return BatchInfo(
                ...
                track_embeddings=event_info.track_embeddings[mask].astype(np.float32),  # NEW passthrough
...
def get_datasets(n_train, n_valid, n_test, 
    ...
    data = TrackDataset(trigger_input_dir=trigger_input_dir,
                        ...
                        use_track_embeddings=use_track_embeddings,  # NEW
                        use_track_hits=use_track_hits,              # NEW
            )
