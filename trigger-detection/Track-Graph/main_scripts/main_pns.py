...
def train(data, model_pnl, model, optimizer, epoch, output_dir, use_wandb=False,  threshold = 0.5):
    train_info = do_epoch(data, model_pnl, model, epoch, optimizer, threshold = threshold)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model_pnl, model, epoch, loss_config=None, threshold=0.5):
    with torch.no_grad():
        val_info = do_epoch(data, model_pnl, model, epoch, optimizer=None, threshold=threshold)
    return val_info


def do_epoch(data, model_pnl, model, epoch, optimizer=None, threshold = 0.5):
    ...
    for batch in data:
        tracks, vtx, partitions_as_graph, n_tracks, trig = batch
        ...
        if tracks.shape[0] == 1:
            skipped_batches += 1
            try:
               pred_x, edge_vals = model_pnl(tracks)
            except ValueError:
                continue
        else:
           pred_x, edge_vals = model_pnl(tracks)
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    #     mlp_params = ((128, 0.1),)
    #     final_pooling = 'mean'
    #     input_dim = 15 + config['data']['use_radius']
    ...
    if config['adj_model']['type'] == 'particlenet-lite-laplace':
        conv_params = (
            (config['adj_model']['k'], tuple(range(15)), (32, 32, 32), 'mean', 'ReLU'),
            (config['adj_model']['k'], tuple(range(15)), (64, 64, 64), 'mean', 'ReLU')
        )
        mlp_params = ((128, 0.1),)
        final_pooling = 'mean'
        config['adj_model']['input_dim'] += 13*config['data']['add_geo_features'] + config['data']['use_radius'] + config['data']['use_pt'] + config['data']['use_pz'] + config['data']['use_predicted_pz']
        model_pnl = ParticleNetLaplace(
                conv_params,
                mlp_params,
                final_pooling,
        ...
        hidden_dim = config['adj_model']['hidden_dim']
        conv_params = (
                (config['adj_model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
                (config['adj_model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
                (config['adj_model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
        )
        mlp_params = ((hidden_dim, 0.1),)
        config['adj_model']['GNN_config']['hidden_dim'] = hidden_dim
        final_pooling = 'mean'
        config['adj_model']['input_dim'] += 13*config['data']['add_geo_features'] + config['data']['use_radius'] + config['data']['use_pt'] + config['data']['use_pz'] + config['data']['use_predicted_pz']
        model_pnl = ParticleNetLaplace(
                conv_params,
    ...
    mconfig['num_features'] += 13*config['data']['add_geo_features'] + config['data']['use_radius'] + config['data']['use_pt'] + config['data']['use_pz'] + config['data']['use_predicted_pz']
    ...
    model_pnl = load_checkpoint(checkpoint_file_pnl, model_pnl, map_location=DEVICE)
    model_pnl.eval()
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        ce_weight = 1
        train_info = train(train_data, model_pnl, model, optimizer, epoch, config['output_dir'], threshold=config['threshold'], 
                )
        table = make_table(
    ...
    #     'aug1/contrast': ['TrackHitDropping', 'NodeDropping', 'EdgeRemoving', 'BackgroundTrackDropping', 'Identity', 'EdgeCorrupting', 'EdgeAdding'],
    #     'aug2/contrast': ['TrackHitDropping', 'NodeDropping', 'EdgeRemoving', 'BackgroundTrackDropping', 'Identity', 'EdgeCorrupting', 'EdgeAdding'],
