...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/particlenet_laplace.yaml')
...
def train(data, model, optimizer, epoch,  output_dir, ):
    train_info = do_epoch(data, model, epoch, optimizer=optimizer, )
...
def do_epoch(data, model, epoch, optimizer=None, ):
    ...
    for batch in data:
        ...
        tracks = tracks.transpose(-1, -2).float().to(DEVICE)
        ...
        preds.extend((torch.nn.Sigmoid()(pred_A)).detach().float().cpu().numpy().flatten())
    ...
    accum_info['correct'] = correct
    accum_info['preds'] = preds
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    if config['model']['type'] == 'particlenet-lite-laplace':
        conv_params = (
            (config['model']['k'], tuple(range(15)), (32, 32, 32), 'mean', 'ReLU'),
            (config['model']['k'], tuple(range(15)), (64, 64, 64), 'mean', 'ReLU')
        )
        mlp_params = ((128, 0.1),)
        final_pooling = 'mean'
        config['model']['input_dim'] += 13*config['data']['add_geo_features'] + config['data']['use_radius'] + config['data']['use_pt'] + config['data']['use_pz'] + config['data']['use_predicted_pz']
        ...
        conv_params = (
                (config['model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
                (config['model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
                (config['model']['k'], tuple(range(15)), (hidden_dim, hidden_dim, hidden_dim), 'mean', 'ReLU'),
        )
        mlp_params = ((hidden_dim, 0.1),)
        config['model']['GNN_config']['hidden_dim'] = hidden_dim
        # conv_params = (
        ...
        #         (config['model']['k'], tuple(range(config['model']['k'])), (256, 256, 256), 'mean', 'ReLU'),
        # )
        # mlp_params = ((256, 0.1),)
        # config['model']['GNN_config']['hidden_dim'] = 256
        final_pooling = 'mean'
        config['model']['input_dim'] += 13*config['data']['add_geo_features'] + config['data']['use_radius'] + config['data']['use_pt'] + config['data']['use_pz'] + config['data']['use_predicted_pz']
        model = ParticleNetLaplace(
                conv_params,
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        # with open(os.path.join(config['output_dir'], f'train_info-{epoch}.pickle'), 'wb') as handle:
        ...
        # with open(os.path.join(config['output_dir'], f'val_info-{epoch}.pickle'), 'wb') as handle:
        #     pickle.dump(val_info, handle, protocol=pickle.HIGHEST_PROTOCOL)
