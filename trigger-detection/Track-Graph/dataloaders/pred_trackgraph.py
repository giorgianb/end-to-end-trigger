...
class TrkDataset():
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(track_vector).to(torch.float), torch.from_numpy(self.origin_vertices[index]).to(torch.float), self.adjs[index], n_tracks, torch.from_numpy(trigger)
...
def get_datasets(n_train1, n_valid1, n_test1, n_train2, n_valid2, n_test2, 
        ...
        use_pt=False, use_pz=False, use_predicted_pz=False):
    ...
    train_data = TrkDataset(input_dir=input_dir, input_dir2=input_dir2, 
                           ...
                           use_pt=use_pt, use_pz=use_pz, use_predicted_pz=use_predicted_pz,\
...
def get_data_loaders(name, batch_size, **data_args):
    if name == 'pred-trackgraph-nomask':
