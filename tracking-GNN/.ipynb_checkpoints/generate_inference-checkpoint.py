...
def data_to_numpy_dict(data_obj):
    ...
    """
    keys_to_try = [
        "x", "edge_index", "edge_attr", "pos", "w", "i", "y", "trigger"
    ]
    out = {}
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f"Infer({args.split})"):
            # Ensure batch_size == 1 semantics; if larger, split to individual graphs
            ...
            for data_obj in data_list:
                ...
                # Forward pass -> logits, probs, pred
                logits = trainer.model(data_obj)
                probs = torch.sigmoid(logits)
                pred = (logits > 0).to(torch.int64)  # matches training threshold (logit>0)

                # Collect inputs & labels (numpy)
                payload = data_to_numpy_dict(data_obj)
                # IDs / filenames
                if "i" in payload:
                    try:
                        sample_id = int(np.atleast_1d(payload["i"])[0])
                    except Exception:
                        sample_id = n_saved
                else:
                    sample_id = n_saved
                ...
                # Save per-sample NPZ (compressed)
