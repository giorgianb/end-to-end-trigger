...
class BatAugmenter(BaseGraphAugmenter):
    ...
    def init_with_data(self, data: pyg.data.Data):
        ...
        train_mask = torch.ones(x.shape[0], dtype=torch.bool).to(x.device)
    ...
    def predict_proba(
        ...
        pred_proba = torch.sigmoid(logits).detach()
        pred_proba = torch.stack([1 - pred_proba, pred_proba], dim=-1)
        if return_numpy:
