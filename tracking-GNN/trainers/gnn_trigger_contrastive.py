...
class SparseGNNTrainer(GNNBaseTrainer):
    ...
    def __init__(self, contrastive_loss_func='TripletMarginLoss', contrastive_loss_weight=1, use_preloaded=False, **kwargs):
        super().__init__(**kwargs)
        self.contrastive_loss_func = getattr(torch.nn, contrastive_loss_func)()
    ...
    def train_epoch(self, data_loader):
        ...
        for i, batch in enumerate(tqdm.tqdm(data_loader)):
            ...
            if self.use_preloaded:
                data_loader.dataset.dataset.preload()
            negatives = data_loader.dataset.dataset.get_negatives(batch, use_preloaded=self.use_preloaded)
            positives = data_loader.dataset.dataset.get_positives(batch, use_preloaded=self.use_preloaded)
            anchor_embeddings = self.model.encode(batch)
            negative_embeddings = self.model.encode(negatives)
            ...
            ce_loss = self.loss_func(torch.sigmoid(batch_output), batch.trigger.float()) 
            contrastive_loss = self.contrastive_loss_func(anchor_embeddings, positive_embeddings, negative_embeddings)

            batch_loss = ce_loss + self.contrastive_loss_weight*contrastive_loss
            batch_loss.backward()
            self.optimizer.step()
