...
class SparseGNNTrainer(GNNBaseTrainer):
    ...
    def train_epoch(self, data_loader):
        ...
        ip_loss_func = nn.functional.mse_loss
        ...
        for i, batch in enumerate(tqdm.tqdm(data_loader, smoothing=0.0)):
            ...
            batch_output, ip_output, trigger_output = self.model(batch)
            mask = batch_output == batch_output
            batch_loss = self.loss_func(batch_output[mask], batch.y.float()[mask], weight=batch.w[mask])
            ip_loss = ip_loss_func(ip_output[:, :2], batch.interaction_point[:, :2]) + 0.01 * ip_loss_func(ip_output[:, 2:], batch.interaction_point[:, 2:])
            trigger_loss = trigger_loss_func(trigger_output, batch.trigger.float())
            batch_loss += trigger_loss + ip_loss
            batch_loss.backward()
            self.optimizer.step()
            ...
            # For IP
            preds_ip.extend(ip_output.cpu().data.numpy())
        ...
        # Summarize the epoch
        n_batches = i + 1
        labels_ip = np.hstack(labels_ip)
        preds_ip = np.hstack(preds_ip)
        ...
        result = {'trigger_prec': metrics.precision_score(labels, preds>0, average='macro'),
            ...
            'explained_variance': metrics.explained_variance_score(labels_ip, preds_ip),
            'max_error': metrics.max_error(labels_ip, preds_ip),
            'rmse': np.sqrt(metrics.mean_squared_error(labels_ip, preds_ip)),
            'mae': metrics.mean_absolute_error(labels_ip, preds_ip),
            'r2': metrics.r2_score(labels_ip, preds_ip)
        }
