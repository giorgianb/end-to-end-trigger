...
import tqdm
from Bat.bat import BatAugmenter
...
class SparseGNNTrainer(GNNBaseTrainer):
    ...
    def train_epoch(self, data_loader):
        ...
        for i, batch in enumerate(tqdm.tqdm(data_loader)):
            ...
            augmenter = BatAugmenter().init_with_data(batch) # Initialize with graph data

            #layer = batch.x[:, 4]
            #batch.active_node[layer >= 3] = 0
            
            x, edge_index, _ = augmenter.augment(self.model, batch.x, batch.edge_index)

            y, train_mask = augmenter.adapt_labels_and_train_mask(batch.active_node, torch.ones_like(batch.active_node).to(torch.bool))
            batch.x = x
            ...
            #train_mask = mvtx_mask
            batch_loss = self.loss_func(torch.sigmoid(batch_output[train_mask]), y.float())
            batch_loss.backward()
            self.optimizer.step()
            sum_loss += batch_loss.item()
            preds.append((batch_output[train_mask]>0).cpu().data.numpy())
            active_node = batch.active_node.cpu().numpy()
            #labels.append(active_node)
    ...
    def evaluate(self, data_loader):
        ...
        if MODE == "active_node":
            ...
            for i, batch in enumerate(tqdm.tqdm(data_loader)):
                ...
                batch_output = self.model(batch)
                batch_loss = self.loss_func(torch.sigmoid(batch_output), batch.active_node.float())
                ...
                for i in range(len(threshold_list)):
                    tp_list[i] += np.logical_and(batch_pred >= threshold_list[i], active_node == 1).sum().item()
                    fp_list[i] += np.logical_and(batch_pred >= threshold_list[i], active_node == 0).sum().item()
                    fn_list[i] += np.logical_and(batch_pred < threshold_list[i], active_node == 1).sum().item()
                    tn_list[i] += np.logical_and(batch_pred < threshold_list[i], active_node ==0).sum().item()
