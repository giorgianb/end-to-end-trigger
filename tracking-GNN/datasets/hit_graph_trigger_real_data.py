...
def load_graph(filename, cylindrical_features_scale, phi_slope_max, z0_max, use_intt, use_cylindrical_std, use_hit_type):
    ...
    n_pixels = 1*np.ones(event_info.hit_cylindrical.shape[0])
    x = np.concatenate([
        event_info.hit_cylindrical/cylindrical_features_scale[None],
        n_pixels.reshape(-1, 1), 
...
class HitGraphDataset(Dataset):
    ...
    def __init__(self, input_dir=None, filelist=None, n_samples=None, real_weight=1.0, n_folders=1, input_dir2=None, phi_slope_max=0.03, z0_max=200, cylindrical_features_scale=(3, 1, 3), load_full_event=False, use_cylindrical_std=False, use_hit_type=False, force_inputdir2_nontrigger=False):
    ...
    def __getitem__(self, index):
        ...
        if self.load_full_event:
            ...
            return torch_geometric.data.Data(
                    ...
                    edge_index=torch.from_numpy(edge_index).to(torch.long),
                    trigger=torch.from_numpy(y).to(torch.long), 
                    trigger_node=torch.from_numpy(event_info.trigger_node).to(torch.bool),
                    i=index, 
                    filename=event_file_name,
            )
...
def get_datasets(n_train, n_valid, input_dir=None, filelist=None, real_weight=1.0, n_folders=1, input_dir2=None, phi_slope_max=0.03, z0_max=200, load_full_event=False, use_cylindrical_std=False, use_hit_type=False, force_inputdir2_nontrigger=False):
    data = HitGraphDataset(input_dir=input_dir, filelist=filelist,
                           n_samples=n_train+n_valid, real_weight=real_weight, n_folders=n_folders, input_dir2=input_dir2, phi_slope_max=phi_slope_max, z0_max=z0_max, load_full_event=load_full_event, use_cylindrical_std=use_cylindrical_std, use_hit_type=use_hit_type, force_inputdir2_nontrigger=force_inputdir2_nontrigger)
