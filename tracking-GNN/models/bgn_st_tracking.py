...
class BPLayerNorm(nn.Module):
    def __init__(self, output_dim):
        super(BPLayerNorm, self).__init__()
        self.intt = gnn.norm.LayerNorm(output_dim)
        self.mvtx = gnn.norm.LayerNorm(output_dim)

    def forward(self, x_intt, x_mvtx):
        return self.intt(x_intt), self.mvtx(x_mvtx)
...
class NodeNetwork(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, hidden_activation='ReLU',
        ...
        if layer_norm:
            layers.append((BPLayerNorm(output_dim), 'x_intt, x_mvtx -> x_intt, x_mvtx'))
...
class GNNSegmentClassifier(nn.Module):
    ...
    """
    def __init__(self, intt_input_dim=3, mvtx_input_dim=3, hidden_dim=8, n_graph_iters=3, n_layers=3, hidden_activation='ReLU', layer_norm=True, phi_slope_max=None, z0_max=None, apply_constraints=False, predict_tracks=True, predict_ip=False, predict_trigger=False, predict_track_features=False, track_features_size=3):
        ...
        self.predict_trigger = predict_trigger
        self.predict_tracks = predict_tracks
        self.predict_track_features = predict_track_features

        if self.predict_trigger:
            self.graph_pred_trigger = make_mlp(hidden_dim, [hidden_dim]*3 + [1],
                                    hidden_activation=hidden_activation,
                                    output_activation=None,
                                    layer_norm=layer_norm)

        if self.predict_ip:
            self.graph_pred_ip = make_mlp(hidden_dim, [hidden_dim]*3 + [3],
                                    hidden_activation=hidden_activation,
                                    output_activation=None,
                                    layer_norm=layer_norm)

        if self.predict_track_features:
            self.pred_track_features = make_mlp(hidden_dim, [hidden_dim]*3 + [track_features_size],
                                    hidden_activation=hidden_activation,
                                    output_activation=None,
                                    layer_norm=layer_norm)
    ...
    def forward(self, inputs):
        ...
        if self.predict_tracks:
            res = torch.zeros(edge_index.shape[1], device=edge_index.device)
            if self.apply_constraints:
                start, end = edge_index
                # Calculate phi_slope, z0 and filter them out
                r2, phi2, z2 = x_intt_0[start][:, :3].T
                ...
                good_seg_mask = torch.ones(edge_index.shape[1], dtype=bool, device=edge_index.device)

            start, end = edge_index
            xp = torch.cat([x_intt[start], x_mvtx[end]], dim=-1)
            res[good_seg_mask] = self.edge_network(xp).squeeze(-1)
            res[~good_seg_mask] = torch.nan
        else:
            res = None

        if self.predict_ip:
            n_batches = inputs.interaction_point.shape[0]

            out_mean = torch.zeros(n_batches, x_intt.shape[1], device=x_intt.device)
            s = scatter_mean(x_intt, inputs.batch_intt, dim=0, out=out_mean)
            ip = self.graph_pred_ip(s)
        else:
            ip = None
        ...
        if self.predict_trigger:
            ...
            out_mean = torch.zeros(n_batches, x_intt.shape[1], device=x_intt.device)
            s = scatter_mean(x_intt, inputs.batch_intt, dim=0, out=out_mean)
            trigger = self.graph_pred_trigger(s).squeeze(-1)
        else:
            trigger = None

        if self.predict_track_features:
            track_features = self.pred_track_features(x_intt)
        else:
            track_features = None


        final = [x for x in [res, ip, track_features, trigger] if x is not None]
        if len(final) == 1:
            return final[0]
        else:
            return final
