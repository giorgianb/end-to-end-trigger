...
class GNNGraphClassifier(nn.Module):
    ...
    def __init__(self, input_dim=3, hidden_dim=8, n_graph_iters=3,
                 hidden_activation='Tanh', layer_norm=True, add_n_hits=False, node_model_path=None):
        super().__init__()
        ...
        if node_model_path is not None:
            config_file = node_model_path + '/config.pkl'
            config = pickle.load(open(config_file, 'rb'))
            model_config = config.get('model', {})
            model_config.pop('loss_func')
            model_config.pop('name')
            from models.agat_active import GNNGraphClassifier
            model = GNNGraphClassifier(**model_config)

            checkpoint_dir = os.path.join(node_model_path, 'checkpoints')
            ...
            self.node_model = load_checkpoint(checkpoint_file, model)
    ...
    def forward(self, inputs):
        ...
        mask = self.node_model(inputs) > 0.5
        start_edge_mask = mask[inputs.edge_index[0]]
        end_edge_mask = mask[inputs.edge_index[1]]
        edge_mask = start_edge_mask & end_edge_mask

        #x = inputs.x[mask]
        batch = inputs.batch[mask]
        edge_index = inputs.edge_index[:, edge_mask]
        x = self.input_network(inputs.x)
        temp = torch.cuda.FloatTensor(batch.shape[0]).fill_(1)
        n_hits = scatter_add(temp, batch).view(-1, 1)
