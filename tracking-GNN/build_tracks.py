...
def load_graph(filename, min_edge_probability=0.5, intt_required=False):
...
INPUT_DIRS = ['tracking_output/trigger/1/', 'tracking_output/nontrigger/0/']
OUTPUT_DIRS = ['tracks/trigger/', 'tracks/nontrigger/']

# Used only for keeping track of progress
...
def main():
    ...
    for input_file_set, output_dir in zip(input_file_sets, OUTPUT_DIRS):
        ...
        with tqdm_joblib(tqdm(desc="Conversion", total=len(input_file_set))) as progress_bar:
                Parallel(n_jobs=16)(delayed(process)(filename, output_dir) for filename in input_file_set)
